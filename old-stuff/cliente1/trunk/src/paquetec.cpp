...
bool cPCCastSpell::serialize(cByteQueue* q){
	q->Write(id);
	q->Write(slot);
	q->Write(x);
	q->Write(y);
...
bool cPCMeditate::serialize(cByteQueue* q){
	q->Write(id);
	return true;
}

bool cPCGetClasesInfo::serialize(cByteQueue* q){
	q->Write(id);
	return true;
}

bool cPCGetRazasInfo::serialize(cByteQueue* q){
	q->Write(id);
	return true;
}

bool cPCGetCiudadesInfo::serialize(cByteQueue* q){
	q->Write(id);
	return true;
}

bool cPCGoToChar::serialize(cByteQueue* q){
	q->Write(id);
	q->WriteString(what.c_str());
	return true;
}

bool cPCRequestSkills::serialize(cByteQueue* q){
	q->Write(id);
	return true;
}

bool cPCModifySkills::serialize(cByteQueue* q){
	q->Write(id);
	for(int i=0;i<21;i++)
	{
		q->Write((*_skills)[i]);
...
bool cPCWarpChar::serialize(cByteQueue* q){
	q->Write(id);
	q->WriteString(what.c_str());
	q->Write(map);
	q->Write(x);
...
bool cPCPing::serialize(cByteQueue* q){
	q->Write(id);
	return true;
}