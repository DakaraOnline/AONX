...
ClienteArgentum::~ClienteArgentum()
{
	delete login_window;//FIXME esto no ir�a aca (??).
...
//	ministats_window= new vministats;
	...
	while (_run)
	{
		...
		if((logged)&&(currentChar>0)){
			ao::Entity_ptr currentEnt = _entities[currentChar];
			
			_engine->render( currentEnt->getPos().x, currentEnt->getPos().y,
					 currentEnt->getOffset().x, currentEnt->getOffset().y );
		}
...
void ClienteArgentum::command_parse(std::string what)
{
	...
	if(1 == 2 ) //stricmp(command.c_str(),"online")==0)
	{
		comu->Online();
	}
}
